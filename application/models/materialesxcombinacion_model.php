...
class materialesxcombinacion_model extends CI_Model {
    ...
    public function getRecords() {
      ...
      ,MXC.Registro AS Registro ", false);
            $this->db->from('sz_MaterialesXCombinacion AS MXC ');
            $this->db->join('sz_Estilos AS E', 'MXC.Estilo = E.ID');
            $this->db->join('sz_Combinaciones AS C', 'MXC.Combinacion = C.ID');
            $this->db->join('sz_MaterialesXCombinacionDetalle AS MXCD', 'MXCD.MaterialXCombinacion = MXC.ID');
...
//            print $str;
    ...
    public function onAgregar($array) {
        try {
            $this->db->insert("sz_MaterialesXCombinacion", $array);
...
//            PRINT "\n ID IN MODEL: $LastIdInserted \n";
    ...
    public function onAgregarDetalle($array) {
        try {
            $this->db->insert("sz_MaterialesXCombinacionDetalle", $array);
        } catch (Exception $exc) {
            echo $exc->getTraceAsString();
        }
    }
    ...
    public function onModificar($ID, $DATA) {
        try {
            $this->db->where('ID', $ID);
            $this->db->update("sz_MaterialesXCombinacion", $DATA);
//            print $str = $this->db->last_query();
    ...
    public function onModificarDetalle($ID, $DATA, $M) {
        try {
            ...
            $this->db->update("sz_MaterialesXCombinacionDetalle", $DATA);
//            print $str = $this->db->last_query();
        } catch (Exception $exc) {
            echo $exc->getTraceAsString();
        }
    ...
    public function getExisteMaterial($Material, $MaterialCombinacion) {
        try {
            ...
            $this->db->from('sz_MaterialesXCombinacionDetalle AS MXC ');
...
//            print $str;
    ...
    public function onEliminar($ID) {
        try {
            ...
            $this->db->where('ID', $ID);
            $this->db->update("sz_MaterialesXCombinacion");
//            print $str = $this->db->last_query();
        } catch (Exception $exc) {
            echo $exc->getTraceAsString();
        }
    ...
    public function onEliminarDetalleInactivo($ID,$M) {
        try { 
            ...
            $this->db->delete('sz_MaterialesXCombinacionDetalle');
//            print $str = $this->db->last_query();
        } catch (Exception $exc) {
            echo $exc->getTraceAsString();
        }
    ...
    public function onEliminarDetalle($ID) {
        try {
            $this->db->set('Estatus', 'INACTIVO');
            $this->db->where('MaterialXCombinacion', $ID);
            $this->db->update("sz_MaterialesXCombinacionDetalle");
//            print $str = $this->db->last_query();
        } catch (Exception $exc) {
            echo $exc->getTraceAsString();
        }
    ...
    public function getMaterialesXCombinacionByID($ID) {
        try {
            ...
            $this->db->from('sz_MaterialesXCombinacion AS M');
    ...
    public function getMaterialesXCombinacionDetalleByID($ID) {
	   ...
	   CONCAT(\'<strong><span class="text-info">\',M.Tipo,\'</span></strong>\') AS Tipo,  
           CONCAT(\'<strong><span class="text-success">\',(M.PrecioLista * MXCD.Consumo),\'</span></strong>\')  AS Importe', false);
            $this->db->from('sz_MaterialesXCombinacionDetalle AS MXCD ');
            $this->db->join('sz_Materiales AS M', 'MXCD.Material = M.ID');
            $this->db->join('sz_Catalogos AS C', 'M.UnidadConsumo = C.ID');
            $this->db->like('C.FieldId', 'UNIDADES');
            $this->db->like('C.Estatus', 'ACTIVO');
