    ...
    <div class="card-body">
        <div class="row">
            <div class="col-sm-6 float-left">
                <legend class="float-left">Gestión de Maquinaria</legend>
            </div>
            ...
            <form id="frmNuevo">
                <div class="row">
                    <div class="col-md-12 float-left">
                        <legend class="float-left">Maquinaria</legend>
                    </div>
                </div>
                <div class="row">
                    <div class="d-none">
                ...
                <div class="row">
                    <div class="col-sm">
                        ...
                        <input type="text" class="form-control form-control-sm" id="NoSerie" name="NoSerie"  >
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm">
...
<!--SCRIPT-->
<script>
    var master_url = base_url + 'index.php/Maquinaria/';
    ...
    function getRecords() {
        ...
        $.ajax({
            ...
            if (data.length > 0) {
                $("#tblRegistros").html(getTable('tblMaquinaria', data));
                $('#tblMaquinaria tfoot th').each(function () {
                    $(this).html('');
                });
                var thead = $('#tblMaquinaria thead th');
                var tfoot = $('#tblMaquinaria tfoot th');
                thead.eq(0).addClass("d-none");
                tfoot.eq(0).addClass("d-none");
                $.each($.find('#tblMaquinaria tbody tr'), function (k, v) {
                    var td = $(v).find("td");
                    td.eq(0).addClass("d-none");
                });
                var tblSelected = $('#tblMaquinaria').DataTable(tableOptions);
                $('#tblMaquinaria_filter input[type=search]').focus();
                $('#tblMaquinaria tbody').on('click', 'tr', function () {
                    $("#tblMaquinaria tbody tr").removeClass("success");
                    $(this).addClass("success");
                    var dtm = tblSelected.row(this).data();
                    temp = parseInt(dtm[0]);
                });
                $('#tblMaquinaria tbody').on('dblclick', 'tr', function () {
                    $("#tblMaquinaria tbody tr").removeClass("success");
                    $(this).addClass("success");
                    var id = this.id;
                    ...
                    if (temp !== 0 && temp !== undefined && temp > 0) {
                        ...
                        $.ajax({
                            url: master_url + 'getMaquinariaByID',
                            type: "POST",
                            dataType: "JSON",
                            data: {
                                ID: temp
